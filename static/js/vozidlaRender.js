...
document.addEventListener("DOMContentLoaded", () => {
  console.log(maxPrice,minPrice,typVozidla);
  if (isNaN(maxPrice) || isNaN(minPrice) || typVozidla == null) {
    ignore = true
  }
  console.log("ignore is: "+ignore);
  
  for (let i = 0; i < auta.length; i++) {
    if (auta[i].displayPrice > maxPrice || auta[i].displayPrice < minPrice || !(auta[i].kategorie.includes(typVozidla)) && ignore == false) {
      console.log("Skipped!",auta[i].displayPrice > maxPrice , auta[i].displayPrice < minPrice , !(auta[i].kategorie.includes(typVozidla)));
      console.log(auta[i].kategorie);
      continue;
    }
  ...
  Array.from(infos).forEach(info => {
    ...
    function resetFilters(e) {
      ...
      document.getElementById('min-price').value = 40;
      document.getElementById('max-price').value = 90;
